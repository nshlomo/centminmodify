    ...
    if [ -f "$LECLIENT_LEBIN" ]; then
      ...
      if [[ "$TOPLEVEL" = [yY] ]]; then
        if [[ "$LECLIENT_LESTAGE" = [yY] ]]; then
          echo "$LECLIENT_LEBIN --force --staging --issue  -w /home/nginx/domains/${vhostname}/public -d ${vhostname} -d www.${vhostname} -k $LECLIENT_LEKEYLENGTH"
          $LECLIENT_LEBIN --force --staging --issue -w "/home/nginx/domains/${vhostname}/public" -d "${vhostname}" -d "www.${vhostname}" -k "$LECLIENT_LEKEYLENGTH"
        else
          echo "$LECLIENT_LEBIN --force --issue  -w /home/nginx/domains/${vhostname}/public -d ${vhostname} -d www.${vhostname} -k $LECLIENT_LEKEYLENGTH"
          $LECLIENT_LEBIN --force --issue -w "/home/nginx/domains/${vhostname}/public" -d "${vhostname}" -d "www.${vhostname}" -k "$LECLIENT_LEKEYLENGTH"
        fi
      else
        if [[ "$LECLIENT_LESTAGE" = [yY] ]]; then
          echo "$LECLIENT_LEBIN --force --staging --issue  -w /home/nginx/domains/${vhostname}/public -d ${vhostname} -k $LECLIENT_LEKEYLENGTH"
          $LECLIENT_LEBIN --force --staging --issue -w "/home/nginx/domains/${vhostname}/public" -d "${vhostname}" -k "$LECLIENT_LEKEYLENGTH"
        else
          echo "$LECLIENT_LEBIN --force --issue  -w /home/nginx/domains/${vhostname}/public -d ${vhostname} -k $LECLIENT_LEKEYLENGTH"
          $LECLIENT_LEBIN --force --issue -w "/home/nginx/domains/${vhostname}/public" -d "${vhostname}" -k "$LECLIENT_LEKEYLENGTH"
        fi
      fi
      LECHECK=$?
      ...
      if [[ "$LECHECK" = '0' ]]; then
        ...
        if [[ -f "${ACME_BINDIR}/${vhostname}/${vhostname}.cer" && -f "${ACME_BINDIR}/${vhostname}/ca.cer" ]]; then
          ls -lah "${ACME_BINDIR}/${vhostname}/"
          echo
          echo "create ${ACME_BINDIR}/${vhostname}/${vhostname}-leunified.crt"
          cat "${ACME_BINDIR}/${vhostname}/${vhostname}.cer" "${ACME_BINDIR}/${vhostname}/ca.cer" > "/usr/local/nginx/conf/ssl/${vhostname}/${vhostname}-leunified.crt"
          ls -lah "/usr/local/nginx/conf/ssl/${vhostname}/${vhostname}-leunified.crt"
          echo
          echo "installcert populate ${ACME_BINDIR}/${vhostname}/${vhostname}.conf"
          $LECLIENT_LEBIN installcert -d "${vhostname}" --certpath "/usr/local/nginx/conf/ssl/${vhostname}/${vhostname}.cer" --keypath "/usr/local/nginx/conf/ssl/${vhostname}/${vhostname}-le.key" --capath "/usr/local/nginx/conf/ssl/${vhostname}/ca.cer" --reloadcmd /usr/bin/ngxreload
          echo
        fi
        ...
        sed -i "s|\/usr\/local\/nginx\/conf\/ssl\/${vhostname}\/${vhostname}.crt|\/usr\/local\/nginx\/conf\/ssl\/${vhostname}\/${vhostname}-leunified.crt|" "/usr/local/nginx/conf/conf.d/${vhostname}.ssl.conf"
        sed -i "s|\/usr\/local\/nginx\/conf\/ssl\/${vhostname}\/${vhostname}.key|\/usr\/local\/nginx\/conf\/ssl\/${vhostname}\/${vhostname}-le.key|" "/usr/local/nginx/conf/conf.d/${vhostname}.ssl.conf"
        ...
        sed -i "s|\/usr\/local\/nginx\/conf\/ssl\/${vhostname}\/${vhostname}-trusted.crt|\/usr\/local\/nginx\/conf\/ssl\/${vhostname}\/${vhostname}-leunified.crt|" "/usr/local/nginx/conf/conf.d/${vhostname}.ssl.conf"
        /usr/bin/nprestart
      fi # LECHECK
    else
